...
- `HomeView.swift`: proje grid'i (LazyVGrid, kart: ikon+isim+path+çalışan servis rozeti; çift tık → aç). Yeni/düzenle proje sheet'i (isim, NSOpenPanel dizin, ikon seçimi). Sağ tık: Düzenle/Sil (onaylı).
